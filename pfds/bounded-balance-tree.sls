...
(library 
  ...
  (begin
    ...
    (define (min tree)
      (cond ((empty? tree)
             (error 'min "Can't take the minimum value of an empty tree"))
    ...
    (define (index tree idx)
      (if (empty? tree)
        (error 'index "No value at index" idx)
    ...
    (define (make-bbtree <)
      (unless (procedure? <) (error "make-bbtree requires a procedure"))
    ...
    (define (bbtree-size bbtree)
      (unless (bbtree? bbtree) (error "bbtree-size requires a bbtree"))
    ...
    (define bbtree-ref
      (let ((ref (lambda (bbtree key failure)
                   (unless (bbtree? bbtree) (error "expected a bbtree"))
        ...
        (case-lambda
          ((bbtree key)
           (define (fail)
             (error 'bbtree-ref "Key is not in the tree" key))
           (ref bbtree key fail))
          ((bbtree key ret)
