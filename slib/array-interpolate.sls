...
(library
  ...
  (begin
    ...
    (define (interpolate-array-ref ra . xs)
      ...
      (define (iar ra xs-in dims-in)
        (define x1 (car xs-in))
        (define b1 (car dims-in))
        (define idx (exact (floor (car xs-in))))
        (define dim-1 (+ -1 (car dims-in)))
        (define xs (cdr xs-in))
        (define dims (cdr dims-in))
