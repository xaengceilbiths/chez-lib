...
(define-library
  ...
  (begin
    ...
    (define (row->anchor pkl row)
      (sprintf #f "<A NAME=\"%s\"></A>" (uri:make-path (take row pkl))))
        ...
        (let ((table:update (or (and (> argc 0) (car args)) (table 'row:update)))
          ...
          (lambda (*keys* *row-hash* . new-row)
            (let* ((new-pkeys (take new-row pkl))
                   ...
                   (same-nonkeys? (equal? (take new-row pkl) (drop row pkl))))
          ...
          (lambda (row)
            ...
            (sprintf #f " <TR>\\n   <TD>%s%s\\n\\n"
                     (string-append
                       ...
                       (html:hidden '*keys* (uri:make-path (take row pkl)))
                       ;; (html:hidden '*suggest* '<>)
                       (car elements)
        ...
        (lambda (row)
          (string-append
            ...
            (if (and edit-point edit-converter
                     (equal? (take edit-point pkl) (take row pkl)))
              (edit-converter row)
              "")))))
    ...
    (define (db->html-files db dir index-filename caption)
      (set! dir (if dir (string-append "" dir) ""))
      (call-with-output-file (string-append dir index-filename)
                             (lambda (port)
        ...
        ((or (catdat 'for-each-row-in-order) (catdat 'for-each-row))
         (lambda (row)
           (call-with-output-file
             (string-append dir (table-name->filename (car row)))
             (lambda (port)
    ...
    ;;returned.
    (define (db->html-directory db dir . index-filename-in) 
      (let ((index-filename (if (null? index-filename-in)
                              "index.html"
                              (car index-filename-in))))
        (if (symbol? dir) (set! dir (symbol->string dir)))
        (if (not (file-exists? dir)) (make-directory dir))
        (db->html-files db dir index-filename dir)
        (path->uri (string-append (current-directory) 
                                  dir 
                                  (if (memq 'windows (features)) "\\" "/")
                                  index-filename))))
