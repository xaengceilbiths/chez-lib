...
(library
  ...
  (begin
    ...
    ;;@end example
    (define (describe-file-lock path . prefix-in)
      (define cep (current-error-port))
      (define w-path (word-lock:path path))
      (define e-path (emacs-lock:path path))
      (let ((prefix (if (null? prefix-in) "" (car prefix-in))))
        (let ((w-cert (word-lock:certificate w-path))
    ...
    (define (windows:user-email-address user hostname)
      (define compname (get-environment-variable "COMPUTERNAME")) ;without domain
      (define workgroup #f)
      (call-with-tmpnam
        (lambda (tmp)
    ...
    (define (user-email-address)
      (define user (or (get-environment-variable "USER") (get-environment-variable "USERNAME")))
      (define hostname (get-environment-variable "HOSTNAME")) ;with domain
