...
(library
  ...
  (export vrml
          ...
          solid:font
          solid:center-row-of
          solid:center-array-of
          solid:center-pile-of
          solid:arrow
          solid:translation
          solid:scale
          solid:rotation)
  ...
  (begin
    ...
    ;;(default 1).
    (define (scene:sun latitude julian-day hour turbidity . strength-in)
      (let* ((theta_s (solar-polar (solar-declination julian-day)
        ...
        (let ((strength (if (null? strength-in) 1 (car strength-in))))
    ...
    (define (scene:overcast latitude julian-day hour turbidity . strength-in)
      (let* ((theta_s (solar-polar (solar-declination julian-day)
                                   latitude
                                   (solar-hour julian-day hour)))
             (sun-chroma (sunlight-chromaticity turbidity (car theta_s)))
    ...
    ;;horizontal.
    (define (scene:viewpoint name distance compass . pitch-in)
      (let ((pitch (* pi/180 (if (null? pitch-in) 0 (car pitch-in)))))
