...
(library
  ...
  (begin
    ...
    (define http:byline (make-parameter #f))
      ...
      (define byline
        (or
          ...
          (sprintf
            ...
            (if (get-environment-variable "SERVER_PROTOCOL") "CGI/1.0" "HTTP/1.0"))))
    ...
    (define (cgi:request-line)
      (define method (get-environment-variable "REQUEST_METHOD"))
      (and method
           (list (string-ci->symbol method)
                 (get-environment-variable "SCRIPT_NAME")
                 (get-environment-variable "SERVER_PROTOCOL"))))

    (define (cgi:query-header)
      (define assqs '())
      (cond ((and (get-environment-variable "SERVER_NAME") (get-environment-variable "SERVER_PORT"))
             (set! assqs (cons (cons 'host (string-append (get-environment-variable "SERVER_NAME")
                                                          ":"
                                                          (get-environment-variable "SERVER_PORT")))
                               assqs))))
      (for-each
        (lambda (envar)
          (define valstr (get-environment-variable envar))
          (if valstr (set! assqs
    ...
    ;; value of @code{(get-environment-variable "REQUEST_METHOD")}.
    (define (cgi:read-query-string)
      (define request-method (get-environment-variable "REQUEST_METHOD"))
    ...
    (define (cgi:serve-query serve-proc)
      (let* ((script-name (get-environment-variable "SCRIPT_NAME"))
    ...
    (define (make-query-alist-command-server rdb command-table . just-params-in?)
      (define comsrvcal (make-command-server rdb command-table))
      (let ((just-params? (if (null? just-params-in?) #f (car just-params-in?))))
        (lambda (query-alist)
          (define comnam #f)
          (define find-command?
            (lambda (cname)
