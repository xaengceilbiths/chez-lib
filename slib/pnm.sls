            ...
            (do ((idx 0 (+ 8 idx)))
              ((>= idx wid8)
               (if (< idx width)
                 (do ((idx idx (+ 1 idx))
                      (bdx 7 (+ -1 bdx))
                      (bts 0 (+ bts (arithmetic-shift (if (array-ref row idx) 1 0) bdx))))
                   ((>= idx width)
                    (write-byte bts port)))))
              (do ((idx idx (+ 1 idx))
                   (bdx 7 (+ -1 bdx))
               ...
               (if (< idx width)
                 (let ((byt (read-byte port)))
                   (do ((idx idx (+ 1 idx))
                        (bdx 7 (+ -1 bdx)))
                     ((>= idx width))
                     (array-set! row (bit-set? bdx byt) idx)))))
              (let ((byt (read-byte port)))
                (array-set! row (bit-set? 7 byt) (+ 0 idx))
                (array-set! row (bit-set? 6 byt) (+ 1 idx))
                (array-set! row (bit-set? 5 byt) (+ 2 idx))
                (array-set! row (bit-set? 4 byt) (+ 3 idx))
                (array-set! row (bit-set? 3 byt) (+ 4 idx))
                (array-set! row (bit-set? 2 byt) (+ 5 idx))
                (array-set! row (bit-set? 1 byt) (+ 6 idx))
                (array-set! row (bit-set? 0 byt) (+ 7 idx)))))))
