...
(define-library
  ...
  (begin
    ...
    (define scheme-source (make-parameter (current-input-port)))
    (define scheme-source-name (make-parameter "stdin"))
    (define derived-txi (make-parameter (current-output-port)))
    (define derived-txi-name (make-parameter "?"))
    ...
    (define schmooz
      (let* ((scheme-file? (filename:match-ci?? "*??scm"))
        ...
        (define (schmooz-scm-file file txi-name)
          ...
          (parameterize ((scheme-source (open-input-file file))
