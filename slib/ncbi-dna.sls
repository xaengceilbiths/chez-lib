...
(define-library
  ...
  (begin
    ...
    (define (codons<-cDNA cDNA)
      ...
      (let loop ((protein '(*N*))
                 (cdx (or start 0)))
        (if (<= len cdx) (error 'reached 'end cdx))
        (let ((codon (string-ci->symbol (string-copy cDNA cdx (+ 3 cdx)))))
    ...
    (define (P<-cDNA cDNA)
      ...
      (let loop ((protein '())
        ...
        (let ((codon (string-ci->symbol (string-copy cDNA cdx (+ 3 cdx)))))
          (define asc (assq codon cDNA:codons))
          (cond ((not asc) (error 'mystery 'codon codon))
    ...
    (define (cDNA:base-count cDNA)
      ...
      (array-for-each (lambda (chr)
                        (case chr
                          ...
                          ((#\t #\T) (set! cnt:t (+ 1 cnt:t)))
                          (else (error 'cDNA:base-count 'unknown 'base chr))))
                      cDNA)
