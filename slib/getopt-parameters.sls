...
(library
  ...
  (begin
    ...
    (define (getopt->parameter-list optnames arities types aliases . description)
      ...
      (let ((progname (list-ref *argv* (+ -1 (option-index))))
            (optlist '())
            (long-opt-list '())
            (optstring #f)
            (pos-args '())
            ...
            (case opt
              ((#\: #\?)
               ...
               (apply parameter-list->getopt-usage
                      progname optnames arities types aliases description))
              ((#f)
               (cond ((and (< (option-index) (length *argv*))
                           (string=? "-" (list-ref *argv* (option-index))))
                      ((option-index) (+ 1 (option-index)))
                      (finish))
                     ((< (option-index) (length *argv*))
                      (let ((topt (assoc curopt aliases)))
                        (if topt (set! curopt (cadr topt)))
                        (cond
                          ((and positional? (not topt))
                           (set! unclaimeds
                             (cons (list-ref *argv* (option-index)) unclaimeds))
                     ...
                     (else (finish))))
              (else
                (when (char? opt) (set! opt (string opt)))
                (let ((topt (assoc opt aliases)))
                  ...
                  (cond
                    ...
                    ((not (can-take-arg? topt))
                     ...
                     (loop))
                    ((option-arg) (set! curopt topt) (adjoin-val (option-arg) curopt) (loop))
                    (else
      ...
      (let ((aliast (map list optnames))
        ...
        (for-each
          (lambda (optname arity aliat)
            (let loop ((initials (filter strlen=1? (remove number? (cdr aliat))))
                       (longname (remove strlen=1? (remove number? (cdr aliat)))))
    ...
    (define (getopt->arglist optnames positions
      ...
      (define progname (list-ref *argv* (+ -1 (option-index))))
