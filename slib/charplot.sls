  ...
  (begin
    ...
    (define (charplot:find-scale isize delta)
      (cond ((zero? delta) (set! delta 1))
            ((inexact? delta) (set! isize (inexact isize))))
      (do ((d 1 (* d 10))
        ...
        (let* ((xscale (charplot:find-scale plot-width (- xmax xmin)))
          ...
          (for-each (lambda (x)
                      ...
                      (if (< -1 idx actual-width)
                        ...
                        (error x (/ (* x (car xscale)) (cadr xscale))
                                    (+ ix-min idx))))
                    data)
          (map list
    ...
    (define (charplot:plot data-in xlabel ylabel . histogram-in?)
      (define clen (string-length char:curves))
      (let* ((histogram? (if (null? histogram-in?) #f (car histogram-in?)))
             (data (charplot:data->lists data-in))
             (pra (charplot:make-array))
    ...
    (define (charplot:plot-function func vlo vhi . npts-in)
      (let ((npts (if (null? npts-in) 64 (car npts-in))))
