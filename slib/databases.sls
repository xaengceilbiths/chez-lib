...
(define-library
  ...
  (begin
    ...
    (define (open-database! filename . type-in)
      (let ((type (and (not (null? type-in)) (car type-in))))
        (let ((dbs #f))
          (dynamic-wind
            (lambda () (set! dbs (mdbm:*databases* #f)))
            (lambda ()
    ...
    (define (open-database rdb . type-in)
      (let ((type (and (not (null? type-in)) (car type-in))))
        (let ((dbs #f))
          (dynamic-wind
            (lambda () (set! dbs (mdbm:*databases* #f)))
    ...
    (define (list-table-definition rdb table-name)
      (cond (((rdb 'table-exists?) table-name)
             (let* ((table ((rdb 'open-table) table-name #f))
               ...
               (list table-name
                     (take coldefs prilimit)
                     (drop coldefs prilimit)
                     ((table 'row:retrieve*)))))
            (else #f)))
