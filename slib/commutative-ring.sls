...
(library
  ...
  (export make-ruleset
          combined-rulesets
          cring:define-rule)
  (import (except (scheme base) zero? + - * /)
          (scheme char)
          (scheme cxr)
          (slib common)
          (slib databases)
    ...
    (define (cr+-args->trms args)
      (let loop ((args args) (cof 1) (numbers 0) (arg_exps '()))
        (cond ((null? args) (cons numbers arg_exps))
              ...
              ((and (is-term-op? (car args) '*) (any number? (cdar args)))
               ;;(print 'got-here (car args) '=> (cons '* (remove number? (cdar args))))
               (set! arg_exps
                 (loop (list (cons '* (remove number? (cdar args))))
                       (apply number* cof (filter number? (cdar args)))
                       numbers
                       arg_exps))
               (loop (cdr args) cof (car arg_exps) (cdr arg_exps)))
              ((is-term-op? (car args) '-)
      ...
      (define (do-terms sign fct_cofs)
        (expression-sort
          (map (lambda (fct_cof)
               ...
               fct_cofs)))
      (let* ((all_cofs (remove (lambda (fct_cof)
                                    (or (zero? (cdr fct_cof))
                                        (eqv? ident (car fct_cof))))
                                  res_cofs))
             (cofs (map cdr all_cofs))
             (some-positive? (any positive? cofs)))
        ;;(print op 'positive? some-positive? 'negative? (any negative? cofs) all_cofs)
        (cond ((and some-positive? (any negative? cofs))
               (append (list inv-op
                             (finish (do-terms
                                       1 (remove negative-cof? all_cofs))))
                       (do-terms -1 (filter negative-cof? all_cofs))))
              (some-positive? (finish (do-terms 1 all_cofs)))
              ((not (any negative? cofs)) ident)
              ...
              ((and (> (abs pow) 1) (cring:apply-rule->terms arg arg))
               => (lambda (terms)
                    ...
                    (if (odd? pow)
                      ...
                      (arg-loop arg_pows))))
              ((or (any try-fct_pow res_pows) (any try-fct_pow arg_pows))
