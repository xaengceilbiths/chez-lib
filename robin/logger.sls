...
(library
  ...
  (begin
    ...
    (define-record-type <logger>
                        (logger port level flag)
                        logger?
                        (port port-get)
                        (level level-get level-set!)
                        (flag created-file?))
    ...
    (define (new-logger out)
      (cond ((output-port? out)
             (logger out 'debug #f))
            ((string? out)
             (logger (open-output-file out) 'debug #t))
            (else
              (error "new-logger requires an output port or filename"))))
    ...
    (define (log-level logger level)
      (if (valid-level? level)
        ...
        (error "log-level given invalid level")))
